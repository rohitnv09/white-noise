  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (let i = 0; i < BAND_COUNT; i++) {
        ...
        const speed = target > smoothed[i] ? 0.3 : 0.08;
        smoothed[i] += (target - smoothed[i]) * speed;
      }

      // Average energy
      let totalEnergy = 0;
      for (let i = 0; i < BAND_COUNT; i++) totalEnergy += smoothed[i];
      ...
      // Soft ambient wash across bottom
      {
        ...
        const grad = ctx!.createLinearGradient(0, height, 0, height - washH);
        ...
        ctx!.fillStyle = grad;
        ctx!.fillRect(0, height - washH, width, washH);
      }
      ...
      for (let i = 0; i < BAND_COUNT; i++) {
        ...
        const grad = ctx!.createRadialGradient(x, height + 15, 0, x, height + 15, radius);
        ...
        ctx!.fillRect(x0, Math.max(0, height - radius), x1 - x0, radius + 15);
      }
      ...
      // Ambient wash
      {
        ...
        const grad = ctx!.createLinearGradient(0, 0, washW, 0);
      ...
      for (let i = 0; i < BAND_COUNT; i++) {
        ...
        const grad = ctx!.createRadialGradient(-12, y, 0, -12, y, radius);
        ...
        ctx!.fillRect(0, y0, radius, y1 - y0);
      }
      ...
      {
        const washW = 120 + avgEnergy * 220;
        const grad = ctx!.createLinearGradient(width, 0, width - washW, 0);
        const [r, g, b] = getColorAtPosition(0.8, timeShift + 0.66);
        grad.addColorStop(0, `rgba(${r}, ${g}, ${b}, ${0.03 + avgEnergy * 0.07})`);
        grad.addColorStop(0.5, `rgba(${r}, ${g}, ${b}, ${avgEnergy * 0.02})`);
        grad.addColorStop(1, 'rgba(0, 0, 0, 0)');
        ctx!.fillStyle = grad;
        ctx!.fillRect(width - washW, 0, washW, height);
      }
      ...
      for (let i = 0; i < BAND_COUNT; i++) {
        ...
        const radius = 65 + val * 180;
        const grad = ctx!.createRadialGradient(width + 12, y, 0, width + 12, y, radius);
        grad.addColorStop(0, `rgba(${r}, ${g}, ${b}, ${0.15 + val * 0.4})`);
        grad.addColorStop(0.35, `rgba(${r}, ${g}, ${b}, ${0.04 + val * 0.12})`);
        grad.addColorStop(0.65, `rgba(${r}, ${g}, ${b}, ${val * 0.03})`);
        grad.addColorStop(1, 'rgba(0, 0, 0, 0)');
        ...
        const y0 = Math.max(0, y - radius);
        const y1 = Math.min(height, y + radius);
        ctx!.fillRect(Math.max(0, width - radius), y0, radius + 12, y1 - y0);
      }
      ...
      // Bottom edge line
      {
        ...
        const lineGlow = ctx!.createLinearGradient(0, height, 0, height - 14);
